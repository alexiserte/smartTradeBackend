...
public void update(String nombre, String vendorName, HashMap<String, ?> atributos) {
    ...
    public List<Producto> getProductsBySeller(String vendorName) {
        ...
        return database.query("SELECT nombre,id_vendedor,id_categoria,descripcion,precio,imagen,fecha_añadido, validado FROM Producto WHERE id_vendedor IN (SELECT id_usuario FROM Vendedor WHERE id_usuario IN (SELECT id FROM Usuario WHERE nickname = ?))", new ProductMapper(), id_vendedor);
    }
